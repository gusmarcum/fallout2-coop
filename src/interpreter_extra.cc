...
// locks it again game_ticks(10) = ten seconds later: the Vault City gate guard's
// "come in" is one (VCGatGrd talk_p_proc -> VIEntDor timed_event_p_proc). A player
// who is still walking through when the window ends gets the door closed in their
// face, the walk stops, and every later click reports "cannot get there" while the
// client may still draw the door open. (The 2026-09-04 Vault City report itself
// turned out to be the stacked Secret Blocking Hex on the approach tile, above.)
//
