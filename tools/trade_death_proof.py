...
a.send("quicksave", 4)
check("quicksave written for the wipe test", count("quicksave -> slot 16 ok") == 1)
admin("kill 1", 3)
check("one death is not a wipe", count("PARTY WIPE") == 0)
admin("kill 0", 8)
check("second death is a wipe", count("PARTY WIPE") == 1)
check("wipe reloads the newest save (slot 16)", count("party wipe -> reload slot 16") == 1 and count("reload slot 16 ok") == 1)
check("everyone told", a.saw("Everyone is dead.") and b.saw("Everyone is dead."))
check("back-to-save announced", a.saw("Back to the last save"))
probe_baseline()
dead_before = count("actor is dead")
